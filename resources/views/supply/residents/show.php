...
<div class="page-header page-header--balanced page-header--premium mb-4">
    <div class="page-header-left">
        <h1 class="h3 page-title mb-1">Suministros · <?= htmlspecialchars((string) ($resident['display_name'] ?? 'Vecino')) ?></h1>
        <p class="page-meta mb-0">
            <?= htmlspecialchars((string) ($resident['community_name'] ?? '—')) ?>
            · <?= htmlspecialchars((string) ($resident['community_city'] ?? '—')) ?>
            <?php if (!empty($resident['unit_label'])): ?>
                · Vivienda <?= htmlspecialchars((string) $resident['unit_label']) ?>
            <?php endif; ?>
        </p>
    </div>
    ...
    <a class="btn btn-outline-secondary btn-sm"
           href="<?= $ab ?>/suministros/vecinos<?= !empty($resident['community_id']) ? '?community_id=' . (int) $resident['community_id'] : '' ?>"
           title="Volver">
...
<div class="tab-content">
                ...
                <table class="table table-hover align-middle mb-0" data-datatable data-page-length="10" data-empty="No hay contratos">
                    ...
                    <?php if ($contracts === []): ?>
                        <tr class="table-empty-row">
                            <td colspan="9" class="text-muted text-center py-4">No hay contratos para este vecino.</td>
                        </tr>
                    <?php endif; ?>
                        ...
                        <tr>
                            ...
                            <td class="text-end text-nowrap">
                                <div class="table-actions actions-cell justify-content-end">
                                    ...
                                    <form method="post"
                                          action="<?= $ab ?>/suministros/vecinos/<?= (int) ($resident['id'] ?? 0) ?>/contratos/<?= (int) ($c['id'] ?? 0) ?>/delete"
                                          class="d-inline"
    ...
    <div class="tab-pane fade <?= $activeNuevo ? 'show active' : '' ?>" id="tab-nuevo" role="tabpanel">
        <div class="card border-0 shadow-sm">
            <div class="card-body">
                ...
                <form method="post"
                      action="<?= $ab ?>/suministros/vecinos/<?= (int) ($resident['id'] ?? 0) ?>/contratos"
                    ...
                    <div class="col-12">
                        ...
                        <?php
                        $defaultSupplyAddress = trim((string) ($old['supply_address'] ?? ''));
                        if ($defaultSupplyAddress === '') {
                            $defaultSupplyAddress = trim((string) ($resident['direccion_postal'] ?? ''));
                        }
                        if ($defaultSupplyAddress === '') {
                            $defaultSupplyAddress = trim((string) (($resident['community_address'] ?? '') . ' ' . ($resident['community_city'] ?? '')));
                        }
                        ?>
                        <textarea class="form-control" name="supply_address" rows="2" required><?= htmlspecialchars($defaultSupplyAddress) ?></textarea>
                    </div>
                    ...
                    <div class="col-12 d-flex justify-content-end gap-2 pt-2">
                        <a href="<?= $ab ?>/suministros/vecinos/<?= (int) ($resident['id'] ?? 0) ?>" class="btn btn-outline-secondary btn-sm">Cancelar</a>
    ...
    (function () {
            ...
            modal?.addEventListener('show.bs.modal', function (event) {
                ...
                form.action = '<?= $ab ?>/suministros/vecinos/<?= (int) ($resident['id'] ?? 0) ?>/contratos/' + encodeURIComponent(contractId) + '/update';
